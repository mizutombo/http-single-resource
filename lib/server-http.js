...
const server = http.createServer(function(req, res) {
  ...
  // validated functionality of GET via Postman
    ...
    promiseLiquor.getLiquorList('liquor') // sander
    .then(function(data) {
  ...
  // else if (req.url === '/liquor/gin' && req.method === 'GET') {
    // promiseLiquor.getLiquorType('liquor/gin.json') // sander
  else if (req.url === '/liquor/:id' && req.method === 'GET') {
    promiseLiquor.getLiquorType('liquor/:id') // sander
    .then(function(data) {
  ...
  else if (req.url === '/liquor/:id' && req.method === 'POST') {
  ...
  else if (req.url === '/liquor/:id' && req.method === 'PUT') {
    bodyReader(req, function(err, data) {
      if(err) {
        res.statusCode = 400;
        res.end(err.message);
        ...
        ss.update('id', data) // ss = storageScout
        .then(function(data) {
          res.setHeader('content-type', 'application/json');
          res.write(data);
          res.end();
  ...
  // else if (req.url === '/liquor/whisky' && req.method === 'DELETE') {
    // promiseLiquor.removeLiquor('liquor/whisky.json') // sander
  else if (req.url === '/liquor/:id' && req.method === 'DELETE') {
    promiseLiquor.removeLiquor('liquor/:id.json') // sander
    .then(function() {
      res.setHeader('content-type', 'text/html; charset=utf-8');
      res.write('removed file');
      res.end();
    })
    .catch(function(err) {
      console.log(err);
